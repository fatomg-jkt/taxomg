...
type Page = "dashboard" | "ppn" | "pph21" | "unifikasi" | "pb1" | "umkm" | "documents" | "financeObsidian" | "finance1001" | "financeResto";
...
const pageMeta: Record<Page, { title: string; subtitle: string; types?: TaxType[] }> = {
  ...
  financeObsidian: { title: "Finance Obsidian", subtitle: "Monitoring cashflow, saldo bank, uang masuk, uang keluar, dan saldo akhir." },
  finance1001: { title: "Finance 1001", subtitle: "Monitoring cashflow, saldo bank, uang masuk, uang keluar, dan saldo akhir." },
  financeResto: { title: "Finance Resto", subtitle: "Monitoring cashflow, saldo bank, uang masuk, uang keluar, dan saldo akhir." },
};
...
const financeNavItems = [
  ["financeObsidian", Landmark, "Obsidian"], ["finance1001", WalletCards, "1001"], ["financeResto", Building2, "Resto"],
] as const;
...
type UpdateSaldoRecord = { id: string; tanggal: string; brand: string; perusahaan: string; bank: string; nomorRekening: string; deskripsi: string; uangMasuk: number; uangKeluar: number; saldoAkhir: number; kategori: string; proyeksiAktual: string; source: "Excel Import" | "Manual Input" | string; keterangan: string; createdAt?: string; updatedAt?: string };
type UpdateSaldoFilters = { tahun: string; bulan: string; brand: string; bank: string; kategori: string };
type FinancePage = "financeObsidian" | "finance1001" | "financeResto";
type UpdateSaldoForm = { id?: string; tanggal: string; brand: string; perusahaan: string; bank: string; nomorRekening: string; deskripsi: string; uangMasuk: string; uangKeluar: string; saldoAkhir: string; kategori: string; proyeksiAktual: string; keterangan: string };
const UPDATE_SALDO_FILTER_STORAGE_KEY = "update-saldo-filters-v1";
const emptyUpdateSaldoForm = (brand = ""): UpdateSaldoForm => ({ tanggal: "", brand, perusahaan: brand, bank: "", nomorRekening: "", deskripsi: "", uangMasuk: "", uangKeluar: "", saldoAkhir: "", kategori: "", proyeksiAktual: "Aktual", keterangan: "" });
function brandName(row: Pick<UpdateSaldoRecord, "brand" | "perusahaan">) { return clean(row.brand) || clean(row.perusahaan) || "Brand Belum Diisi"; }
function normalizeSaldoDate(value: unknown) { const text = clean(value); if (!text) return "-"; if (typeof value === "number") { const d = XLSX.SSF.parse_date_code(value); if (d) return `${d.y}-${String(d.m).padStart(2, "0")}-${String(d.d).padStart(2, "0")}`; } const parsed = new Date(text); if (!Number.isNaN(parsed.getTime())) return `${parsed.getFullYear()}-${String(parsed.getMonth() + 1).padStart(2, "0")}-${String(parsed.getDate()).padStart(2, "0")}`; return text; }
function updateSaldoMonth(row: UpdateSaldoRecord) { const d = new Date(row.tanggal); return Number.isNaN(d.getTime()) ? -1 : d.getMonth(); }
function normalizeUpdateSaldoRecord(row: Partial<Omit<UpdateSaldoRecord, "uangMasuk" | "uangKeluar" | "saldoAkhir">> & { uangMasuk?: unknown; uangKeluar?: unknown; saldoAkhir?: unknown }, index = 0): UpdateSaldoRecord { void index; return { id: clean(row.id) || `update-saldo-${crypto.randomUUID()}`, tanggal: normalizeSaldoDate(row.tanggal), brand: clean(row.brand || row.perusahaan), perusahaan: clean(row.perusahaan || row.brand), bank: clean(row.bank) || "Bank Belum Diisi", nomorRekening: clean(row.nomorRekening), deskripsi: clean(row.deskripsi), uangMasuk: numberValue(row.uangMasuk), uangKeluar: numberValue(row.uangKeluar), saldoAkhir: numberValue(row.saldoAkhir), kategori: clean(row.kategori) || "Belum Dikategorikan", proyeksiAktual: clean((row as { proyeksiAktual?: unknown }).proyeksiAktual) || "Aktual", source: row.source || "Excel Import", keterangan: clean(row.keterangan), createdAt: clean(row.createdAt) || new Date().toISOString(), updatedAt: clean(row.updatedAt) || new Date().toISOString() }; }
function updateSaldoFromForm(form: UpdateSaldoForm): UpdateSaldoRecord { const now = new Date().toISOString(); return normalizeUpdateSaldoRecord({ ...form, id: form.id, brand: clean(form.brand) || clean(form.perusahaan), perusahaan: clean(form.perusahaan) || clean(form.brand), proyeksiAktual: clean(form.proyeksiAktual) || "Aktual", source: "Manual Input", createdAt: now, updatedAt: now }); }
function parseUpdateSaldoWorkbook(buffer: ArrayBuffer, brandScope = ""): UpdateSaldoRecord[] { const wb = XLSX.read(buffer, { type: "array", cellDates: true }); return wb.SheetNames.flatMap((sheet) => { const rows = XLSX.utils.sheet_to_json<Record<string, unknown>>(wb.Sheets[sheet], { defval: "" }); return rows.flatMap((row, index) => { const get = (...names: string[]) => Object.entries(row).find(([key]) => names.some((name) => key.toLowerCase().trim() === name.toLowerCase()))?.[1]; const brand = brandScope || clean(get("Brand")); const perusahaan = brandScope || clean(get("Perusahaan")); const masuk = numberValue(get("Uang Masuk")); const keluar = numberValue(get("Uang Keluar")); const saldo = numberValue(get("Saldo Akhir")); const anyText = Object.values(row).some((value) => clean(value)); if (!anyText) return []; return [normalizeUpdateSaldoRecord({ id: `update-saldo-${sheet}-${index}-${crypto.randomUUID()}`, tanggal: normalizeSaldoDate(get("Tanggal")), brand: brand || perusahaan, perusahaan: perusahaan || brand, bank: clean(get("Bank")), nomorRekening: clean(get("Nomor Rekening")), deskripsi: clean(get("Deskripsi")), uangMasuk: masuk, uangKeluar: keluar, saldoAkhir: saldo, kategori: clean(get("Kategori")) || "Belum Dikategorikan", proyeksiAktual: clean(get("Proyeksi Aktual", "Proyeksi / Aktual")) || "Aktual", source: "Excel Import", keterangan: clean(get("Keterangan")) }, index)]; }); }); }
...
function isFinancePage(page: Page): page is FinancePage { return ["financeObsidian", "finance1001", "financeResto"].includes(page); }
function financeBrand(page: FinancePage) { return page === "financeObsidian" ? "Obsidian" : page === "finance1001" ? "1001" : "Resto"; }
...
export function TaxCoordinatorDashboard() {
  ...
  const [updateSaldoRecords, setUpdateSaldoRecords] = useState<UpdateSaldoRecord[]>([]);
  const [updateSaldoLastSaved, setUpdateSaldoLastSaved] = useState<string | null>(null);
  const [updateSaldoFilters, setUpdateSaldoFilters] = useState<UpdateSaldoFilters>({ tahun: DEFAULT_DASHBOARD_YEAR, bulan: ALL, brand: ALL, bank: ALL, kategori: ALL });
  const [updateSaldoModalOpen, setUpdateSaldoModalOpen] = useState(false);
  const [updateSaldoForm, setUpdateSaldoForm] = useState<UpdateSaldoForm>(emptyUpdateSaldoForm());
  ...
  async function loadUpdateSaldoData() {
    ...
    const payload = await response.json().catch(() => ({ records: [], updatedAt: null }));
    setUpdateSaldoRecords(Array.isArray(payload.records) ? payload.records.map(normalizeUpdateSaldoRecord) : []);
    setUpdateSaldoLastSaved(payload.updatedAt ?? null);
  }
  async function saveUpdateSaldoToCloud() {
    const password = await verifyPassword(); if (!password) return;
    setBusy(true); setError("");
    try {
      const response = await fetch("/api/update-saldo-data", { method: "POST", headers: { "Content-Type": "application/json", "x-dashboard-password": password }, body: JSON.stringify({ records: updateSaldoRecords }) });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.error || "Save to Cloud Update Saldo gagal.");
      setUpdateSaldoLastSaved(payload.updatedAt); setMessage("Save to Cloud Dashboard Finance berhasil. Data tersimpan terpisah dari data pajak.");
    } catch (err) { setError(err instanceof Error ? err.message : "Save to Cloud Update Saldo gagal."); } finally { setBusy(false); }
  }
  async function saveUpdateSaldoManual() { const password = await verifyPassword(); if (!password) return; setUpdateSaldoRecords((rows) => updateSaldoForm.id ? rows.map((row) => row.id === updateSaldoForm.id ? updateSaldoFromForm(updateSaldoForm) : row) : [...rows, updateSaldoFromForm(updateSaldoForm)]); setUpdateSaldoModalOpen(false); setUpdateSaldoForm(emptyUpdateSaldoForm(isFinancePage(page) ? financeBrand(page) : "")); setMessage("Data Dashboard Finance manual ditambahkan. Klik Save to Cloud agar tersimpan."); }
  async function deleteUpdateSaldo(id: string) { const password = await verifyPassword(); if (!password) return; if (!confirm("Hapus data Update Saldo ini?")) return; setUpdateSaldoRecords((rows) => rows.filter((row) => row.id !== id)); setMessage("Data Dashboard Finance dihapus dari state. Klik Save to Cloud agar tersimpan."); }
  function importUpdateSaldoExcel(event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0]; if (!file) return;
    const reader = new FileReader();
    reader.onload = async () => { const password = await verifyPassword(); if (!password) { event.target.value = ""; return; } setBusy(true); try { const rows = parseUpdateSaldoWorkbook(reader.result as ArrayBuffer, isFinancePage(page) ? financeBrand(page) : ""); setUpdateSaldoRecords((current) => [...current, ...rows]); setMessage(rows.length ? `Upload Excel Dashboard Finance berhasil: ${rows.length} baris.` : "Excel Dashboard Finance kosong. Tidak ada data yang ditambahkan."); } catch (err) { setError(err instanceof Error ? err.message : "Upload Excel Dashboard Finance gagal."); } finally { setBusy(false); event.target.value = ""; } };
  ...
  useEffect(() => { const savedSaldo = localStorage.getItem(UPDATE_SALDO_FILTER_STORAGE_KEY); if (savedSaldo) setUpdateSaldoFilters({ ...updateSaldoFilters, ...JSON.parse(savedSaldo) }); const saved = localStorage.getItem(FILTER_STORAGE_KEY); if (saved) { const parsed = JSON.parse(saved) as Partial<Filters>; setFilters({ tahun: parsed.tahun === ALL ? ALL : normalizeYear(parsed.tahun), masaPajak: parsed.masaPajak && (parsed.masaPajak === ALL || monthIndex(parsed.masaPajak) >= 0) ? parsed.masaPajak : ALL, perusahaan: parsed.perusahaan ?? ALL, jenisPajak: parsed.jenisPajak ?? ALL, status: parsed.status ?? ALL, search: parsed.search ?? "" }); } refreshData(); }, []);
  useEffect(() => localStorage.setItem(FILTER_STORAGE_KEY, JSON.stringify(filters)), [filters]);
  useEffect(() => localStorage.setItem(UPDATE_SALDO_FILTER_STORAGE_KEY, JSON.stringify(updateSaldoFilters)), [updateSaldoFilters]);
  ...
  const updateSaldoFiltered = useMemo(() => updateSaldoRecords.filter((r) => { const d = new Date(r.tanggal); const year = Number.isNaN(d.getTime()) ? DEFAULT_DASHBOARD_YEAR : String(d.getFullYear()); const scopedBrand = isFinancePage(page) ? financeBrand(page) : updateSaldoFilters.brand; return (updateSaldoFilters.tahun === ALL || year === updateSaldoFilters.tahun) && (updateSaldoFilters.bulan === ALL || updateSaldoMonth(r) === monthIndex(updateSaldoFilters.bulan)) && (scopedBrand === ALL || brandName(r) === scopedBrand) && (updateSaldoFilters.bank === ALL || r.bank === updateSaldoFilters.bank) && (updateSaldoFilters.kategori === ALL || r.kategori === updateSaldoFilters.kategori); }), [updateSaldoRecords, updateSaldoFilters, page]);
  const updateSaldoOptions = { tahun: [DEFAULT_DASHBOARD_YEAR], brand: Array.from(new Set(updateSaldoRecords.map(brandName))).filter(Boolean).sort(), bank: Array.from(new Set(updateSaldoRecords.map((r) => r.bank))).filter(Boolean).sort(), kategori: Array.from(new Set(updateSaldoRecords.map((r) => r.kategori))).filter(Boolean).sort() };
...
function UpdateSaldoFilterBar({ filters, setFilters, options, onUpload, onManual, onSave, saving }: { filters: UpdateSaldoFilters; setFilters: (filters: UpdateSaldoFilters) => void; options: { tahun: string[]; brand: string[]; bank: string[]; kategori: string[] }; onUpload: () => void; onManual: () => void; onSave: () => void; saving: boolean }) {
  const set = (key: keyof UpdateSaldoFilters, value: string) => setFilters({ ...filters, [key]: value });
  return <Card className="rounded-3xl border-[#D8E0EA] shadow-sm"><CardContent className="flex flex-wrap items-center gap-3 p-4"><Select value={filters.tahun} onChange={(e) => set("tahun", e.target.value)} className="h-11 flex-1 basis-full rounded-2xl bg-white sm:basis-40"><option value={ALL}>Semua Tahun</option>{options.tahun.map((v) => <option key={v} value={v}>{v}</option>)}</Select><Select value={filters.bulan} onChange={(e) => set("bulan", e.target.value)} className="h-11 flex-1 basis-full rounded-2xl bg-white sm:basis-40"><option value={ALL}>Semua Bulan</option>{MONTH_NAMES.map((v) => <option key={v} value={v}>{v}</option>)}</Select><Select value={filters.bank} onChange={(e) => set("bank", e.target.value)} className="h-11 flex-1 basis-full rounded-2xl bg-white sm:basis-40"><option value={ALL}>Semua Bank</option>{options.bank.map((v) => <option key={v} value={v}>{v}</option>)}</Select><Select value={filters.kategori} onChange={(e) => set("kategori", e.target.value)} className="h-11 flex-1 basis-full rounded-2xl bg-white sm:basis-48"><option value={ALL}>Semua Kategori</option>{options.kategori.map((v) => <option key={v} value={v}>{v}</option>)}</Select><div className="flex w-full flex-wrap gap-3 sm:w-auto sm:flex-nowrap"><Button onClick={onUpload} className="h-11 rounded-2xl bg-blue-600 font-bold hover:bg-blue-700"><Upload className="h-4 w-4" /> Upload Excel</Button><Button onClick={onManual} variant="outline" className="h-11 rounded-2xl font-bold"><Plus className="h-4 w-4" /> Manual</Button><Button onClick={onSave} disabled={saving} variant="outline" className="h-11 rounded-2xl font-bold">Save to Cloud</Button></div></CardContent></Card>;
}
function UpdateSaldoDashboard({ rows, allRows, lastSaved, onUpload, onManual, onDelete }: { rows: UpdateSaldoRecord[]; allRows: UpdateSaldoRecord[]; lastSaved: string | null; onUpload: () => void; onManual: () => void; onDelete: (id: string) => void }) {
  const saldoAwal = rows.reduce((a, r) => a + (r.saldoAkhir - r.uangMasuk + r.uangKeluar), 0);
  const totalMasuk = rows.reduce((a, r) => a + r.uangMasuk, 0); const totalKeluar = rows.reduce((a, r) => a + r.uangKeluar, 0); const totalSaldo = rows.reduce((a, r) => a + r.saldoAkhir, 0);
  const banks = Array.from(new Set(rows.map((r) => r.bank).filter(Boolean))); const brands = Array.from(new Set(rows.map(brandName).filter(Boolean)));
  const monthly = MONTH_NAMES.map((month, index) => ({ month, masuk: rows.filter((r) => updateSaldoMonth(r) === index).reduce((a, r) => a + r.uangMasuk, 0), keluar: rows.filter((r) => updateSaldoMonth(r) === index).reduce((a, r) => a + r.uangKeluar, 0), saldo: rows.filter((r) => updateSaldoMonth(r) === index).reduce((a, r) => a + r.saldoAkhir, 0) }));
  const categorySpend = Array.from(new Set(rows.map((r) => r.kategori).filter(Boolean))).map((kategori) => ({ name: kategori, value: rows.filter((r) => r.kategori === kategori).reduce((a, r) => a + r.uangKeluar, 0) }));
  const bankSaldo = banks.map((bank) => ({ name: bank, value: rows.filter((r) => r.bank === bank).reduce((a, r) => a + r.saldoAkhir, 0) }));
  const brandSaldo = brands.map((brand) => ({ name: brand, value: rows.filter((r) => brandName(r) === brand).reduce((a, r) => a + r.saldoAkhir, 0) }));
  const colors = ["#2563eb", "#16a34a", "#f97316", "#7c3aed", "#dc2626", "#0891b2"];
  return <div className="space-y-6"><div className="rounded-2xl border border-blue-100 bg-white p-4 text-sm font-semibold text-slate-700 shadow-sm"><FileSpreadsheet className="mr-2 inline h-4 w-4 text-blue-600" />Data Dashboard Finance tersimpan terpisah dari data pajak dengan key financeDashboardData. {lastSaved && <span className="ml-2 text-slate-500">Last saved: {new Date(lastSaved).toLocaleString("id-ID")}</span>}</div><section className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">{[{ label: "Saldo Awal", value: saldoAwal, money: true }, { label: "Total Uang Masuk", value: totalMasuk, money: true }, { label: "Total Uang Keluar", value: totalKeluar, money: true }, { label: "Saldo Akhir", value: totalSaldo, money: true }, { label: "Proyeksi Cashflow", value: rows.filter((r) => r.proyeksiAktual.toLowerCase().includes("proyeksi")).reduce((a, r) => a + r.uangMasuk - r.uangKeluar, 0), money: true }, { label: "Selisih Aktual vs Proyeksi", value: rows.reduce((a, r) => a + (r.proyeksiAktual.toLowerCase().includes("proyeksi") ? -(r.uangMasuk - r.uangKeluar) : r.uangMasuk - r.uangKeluar), 0), money: true }].map((item) => <Card key={item.label} className="rounded-3xl border-[#D8E0EA] bg-white shadow-sm"><CardContent className="p-5"><p className="text-xs font-extrabold uppercase text-slate-500">{item.label}</p><p className="mt-3 text-2xl font-black">{item.money ? rupiah(item.value) : plainNumber(item.value)}</p></CardContent></Card>)}</section><section className="grid gap-4 xl:grid-cols-3"><Card className="rounded-3xl border-[#D8E0EA] bg-white shadow-sm"><CardHeader><CardTitle>Cash In vs Cash Out</CardTitle><CardDescription>Uang masuk dan keluar per bulan.</CardDescription></CardHeader><CardContent className="h-72">{rows.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={monthly}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="month" /><YAxis tickFormatter={(v: number) => `${Math.round(v / 1000000)} jt`} /><Tooltip formatter={(v: number) => rupiah(v)} /><Legend /><Bar dataKey="masuk" name="Uang Masuk" fill="#16a34a" radius={[8,8,0,0]} /><Bar dataKey="keluar" name="Uang Keluar" fill="#dc2626" radius={[8,8,0,0]} /></BarChart></ResponsiveContainer> : <EmptySaldoState />}</CardContent></Card><Card className="rounded-3xl border-[#D8E0EA] bg-white shadow-sm"><CardHeader><CardTitle>Saldo Akhir per Bulan</CardTitle><CardDescription>Tren saldo akhir semua brand.</CardDescription></CardHeader><CardContent className="h-72">{rows.length ? <ResponsiveContainer width="100%" height="100%"><AreaChart data={monthly}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="month" /><YAxis tickFormatter={(v: number) => `${Math.round(v / 1000000)} jt`} /><Tooltip formatter={(v: number) => rupiah(v)} /><Area type="monotone" dataKey="saldo" name="Saldo Akhir" stroke="#2563eb" fill="#bfdbfe" strokeWidth={3} /></AreaChart></ResponsiveContainer> : <EmptySaldoState />}</CardContent></Card><Card className="rounded-3xl border-[#D8E0EA] bg-white shadow-sm"><CardHeader><CardTitle>Komposisi Pengeluaran per Kategori</CardTitle><CardDescription>Distribusi uang keluar per kategori.</CardDescription></CardHeader><CardContent className="h-72">{categorySpend.length ? <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={categorySpend} dataKey="value" nameKey="name" innerRadius="55%" outerRadius="78%">{categorySpend.map((entry, i) => <Cell key={entry.name} fill={colors[i % colors.length]} />)}</Pie><Tooltip formatter={(v: number) => rupiah(v)} /><Legend /></PieChart></ResponsiveContainer> : <EmptySaldoState />}</CardContent></Card></section><section className="grid gap-4 xl:grid-cols-2"><SaldoMiniTable title="Saldo per Bank" rows={bankSaldo} /><SaldoMiniTable title="Saldo per Brand / Perusahaan" rows={brandSaldo} /></section><UpdateSaldoTable rows={rows} onUpload={onUpload} onManual={onManual} onDelete={onDelete} totalRows={allRows.length} /></div>;
}
function EmptySaldoState() { return <div className="grid h-full place-items-center rounded-2xl border border-dashed border-slate-300 text-sm font-semibold text-slate-500">Belum ada data sesuai filter.</div>; }
function SaldoMiniTable({ title, rows }: { title: string; rows: { name: string; value: number }[] }) { return <Card className="rounded-3xl border-[#D8E0EA] bg-white shadow-sm"><CardHeader><CardTitle>{title}</CardTitle></CardHeader><CardContent className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Nama</TableHead><TableHead>Saldo Akhir</TableHead></TableRow></TableHeader><TableBody>{rows.length ? rows.map((r) => <TableRow key={r.name}><TableCell className="font-semibold">{r.name}</TableCell><TableCell>{rupiah(r.value)}</TableCell></TableRow>) : <TableRow><TableCell colSpan={2} className="h-20 text-center text-slate-500">Belum ada data.</TableCell></TableRow>}</TableBody></Table></CardContent></Card>; }
function UpdateSaldoTable({ rows, onUpload, onManual, onDelete, totalRows }: { rows: UpdateSaldoRecord[]; onUpload: () => void; onManual: () => void; onDelete: (id: string) => void; totalRows: number }) { const headers = ["Tanggal", "Bank", "Deskripsi", "Kategori", "Uang Masuk", "Uang Keluar", "Saldo Akhir", "Proyeksi / Aktual", "Keterangan", "Source", "Hapus"]; return <Card className="rounded-3xl border-[#D8E0EA] bg-white shadow-sm"><CardHeader><CardTitle>Tabel Mutasi Bank</CardTitle><CardDescription>{rows.length} baris tampil dari {totalRows} total data.</CardDescription></CardHeader><CardContent className="overflow-x-auto"><Table><TableHeader><TableRow>{headers.map((h) => <TableHead key={h} className="whitespace-nowrap text-xs uppercase text-slate-500">{h}</TableHead>)}</TableRow></TableHeader><TableBody>{rows.length ? rows.map((r) => <TableRow key={r.id}><TableCell>{r.tanggal || "-"}</TableCell><TableCell>{r.bank}</TableCell><TableCell className="min-w-64">{r.deskripsi || "-"}</TableCell><TableCell>{r.kategori}</TableCell><TableCell className="text-emerald-700">{rupiah(r.uangMasuk)}</TableCell><TableCell className="text-red-600">{rupiah(r.uangKeluar)}</TableCell><TableCell className="font-bold">{rupiah(r.saldoAkhir)}</TableCell><TableCell>{r.proyeksiAktual || "Aktual"}</TableCell><TableCell className="min-w-52">{r.keterangan || "-"}</TableCell><TableCell><Badge variant={r.source === "Manual Input" ? "success" : "secondary"}>{r.source}</Badge></TableCell><TableCell><Button size="sm" variant="outline" className="rounded-xl text-red-600" onClick={() => onDelete(r.id)}><Trash2 className="h-3 w-3" /> Hapus</Button></TableCell></TableRow>) : <TableRow><TableCell colSpan={headers.length} className="h-36 text-center"><div className="space-y-4 text-sm font-semibold text-slate-500"><p>Belum ada data mutasi. Upload Excel atau tambah manual.</p><div className="flex justify-center gap-3"><Button onClick={onUpload} className="rounded-2xl bg-blue-600"><Upload className="h-4 w-4" /> Upload Excel</Button><Button onClick={onManual} variant="outline" className="rounded-2xl"><Plus className="h-4 w-4" /> Tambah Manual</Button></div></div></TableCell></TableRow>}</TableBody></Table></CardContent></Card>; }
function UpdateSaldoManualModal({ brand, form, setForm, onClose, onSave, saving }: { brand: string; form: UpdateSaldoForm; setForm: (form: UpdateSaldoForm) => void; onClose: () => void; onSave: () => void; saving: boolean }) { const set = (key: keyof UpdateSaldoForm, value: string) => setForm({ ...form, [key]: value }); const field = (key: keyof UpdateSaldoForm, label: string, type = "text") => <div><label className="text-xs font-extrabold uppercase text-slate-500">{label}</label><Input type={type} value={String(form[key] ?? "")} onChange={(e) => set(key, e.target.value)} min={type === "date" ? PAYMENT_DATE_MIN : undefined} max={type === "date" ? PAYMENT_DATE_MAX : undefined} className="mt-1 h-11 rounded-2xl" /></div>; return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4"><div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl"><div className="mb-5 flex items-center justify-between"><div><h2 className="text-2xl font-black">Input Manual Finance {brand}</h2><p className="text-sm font-medium text-slate-500">Source otomatis Manual Input dan tersimpan terpisah dari data pajak.</p></div><Button variant="ghost" size="icon" onClick={onClose}><X className="h-5 w-5" /></Button></div><div className="grid gap-4 md:grid-cols-2">{field("tanggal", "Tanggal", "date")}<input type="hidden" value={form.brand} readOnly />{field("bank", "Bank")}{field("deskripsi", "Deskripsi")}{field("kategori", "Kategori")}{field("uangMasuk", "Uang Masuk")}{field("uangKeluar", "Uang Keluar")}{field("saldoAkhir", "Saldo Akhir")}<div><label className="text-xs font-extrabold uppercase text-slate-500">Proyeksi / Aktual</label><Select value={form.proyeksiAktual} onChange={(e) => set("proyeksiAktual", e.target.value)} className="mt-1 h-11 rounded-2xl"><option value="Aktual">Aktual</option><option value="Proyeksi">Proyeksi</option></Select></div><div className="md:col-span-2">{field("keterangan", "Keterangan")}</div></div><div className="mt-6 flex justify-end gap-3"><Button variant="outline" className="rounded-2xl" onClick={onClose}>Batal</Button><Button className="rounded-2xl bg-blue-600 font-bold hover:bg-blue-700" onClick={onSave} disabled={saving}>{saving ? "Menyimpan..." : "Simpan"}</Button></div></div></div>; }
