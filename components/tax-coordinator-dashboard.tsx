...
export function TaxCoordinatorDashboard() {
  ...
  function addFinanceAccount(brand = "") {
    const account = normalizeFinanceAccount({ brand: brand || "Obsidian", group: "Default", entity: "Entity Belum Diisi", accountName: "", provider: "", accountNumber: "", accountType: "Bank", balance: 0, source: "Manual Input" });
...
function BrandDetails({ accounts, scopedBrand, onAddAccount, onUpdateAccount, onDeleteAccount }: { accounts: FinanceAccount[]; scopedBrand: string; onAddAccount: (brand?: string) => void; onUpdateAccount: (id: string, patch: Partial<FinanceAccount>) => void; onDeleteAccount: (id: string) => void }) { const brands = Array.from(new Set([...(scopedBrand ? [scopedBrand] : DEFAULT_FINANCE_BRANDS), ...accounts.map(a=>a.brand)])); return <div className="space-y-4">{brands.map((brand)=>{ const rows=accounts.filter(a=>a.brand===brand); const customStyle = brandHeaderStyle(brand); return <Card key={brand} className="overflow-hidden rounded-3xl border-[#D8E0EA] bg-white shadow-sm"><CardHeader style={customStyle} className={cn(!customStyle && "bg-gradient-to-r from-slate-900 to-blue-700", "text-white")}><div className="flex flex-wrap items-center justify-between gap-3"><div><CardTitle>{brand}</CardTitle><CardDescription className="text-white/80">{plainNumber(rows.length)} rekening/akun</CardDescription></div><div className="text-right text-2xl font-black">{rupiah(rows.reduce((a,r)=>a+r.balance,0))}</div></div></CardHeader><CardContent className="space-y-4 p-5">{rows.length ? Array.from(new Set(rows.map(r=>r.group))).map(group=><div key={group} className="rounded-2xl border border-slate-200 p-4"><h3 className="mb-3 font-black text-slate-800">{group}</h3>{Array.from(new Set(rows.filter(r=>r.group===group).map(r=>r.entity))).map(entity=><div key={entity} className="mb-4"><p className="mb-2 text-sm font-extrabold text-slate-500">{entity}</p><FinanceAccountRows rows={rows.filter(r=>r.group===group&&r.entity===entity)} onUpdate={onUpdateAccount} onDelete={onDeleteAccount}/></div>)}</div>) : <EmptySaldoState />}<Button onClick={() => onAddAccount(brand)} variant="outline" className="rounded-2xl font-bold"><Plus className="h-4 w-4"/> Tambah Rekening</Button></CardContent></Card>})}</div>; }
