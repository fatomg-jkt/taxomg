...
const MANUAL_STORAGE_KEY = "tax-dashboard-manual-records-v1";
...
type Filters = { tahun: string; masaPajak: string; perusahaan: string; jenisPajak: string; status: string; sourceData: string; sourceSheet: string; search: string };
...
function fromStaticEntry(row: StaticTaxEntry, index = 0): TaxTransaction {
  const pajak = row.pajakTerhutang ?? row.pajak ?? 0;
  const statusAuto = row.statusAuto ?? row.status_auto ?? automaticStatus(numberValue(pajak), row.ntpnNtpd ?? row.ntpn_ntpd ?? "", row.keterangan ?? "", numberValue(row.dpp));
  return { id: row.id || `static-${index}`, perusahaan: row.perusahaan || "Perusahaan Belum Diisi", tahun: row.tahun || periodYear(row.masaPajak ?? row.masa_pajak ?? ""), masaPajak: row.masaPajak ?? row.masa_pajak ?? "-", jenisPajak: row.jenisPajak ?? row.jenis_pajak ?? "PPh Pasal 21", dpp: numberValue(row.dpp), pajakTerhutang: numberValue(pajak), ntpnNtpd: row.ntpnNtpd ?? row.ntpn_ntpd ?? "", tanggalBayar: row.tanggalBayar ?? row.tanggal_bayar ?? "", status: row.status || displayStatus(statusAuto), statusAuto, keterangan: row.keterangan || "", sourceData: row.sourceData ?? row.source_data ?? "Static File", sourceSheet: row.sourceSheet ?? row.source_sheet ?? "tax-data.json", sourceRow: Number(row.sourceRow ?? row.source_row) || index + 1, uploadBatchId: row.uploadBatchId ?? row.upload_batch_id ?? null, createdAt: row.createdAt ?? row.created_at, updatedAt: row.updatedAt ?? row.updated_at };
}
...
async function loadStaticTaxData() {
  ...
  return Array.isArray(payload) ? payload.map(fromStaticEntry) : [];
}
...
function toCsv(rows: TaxTransaction[]) {
  const headers = ["id", "perusahaan", "tahun", "masaPajak", "jenisPajak", "dpp", "pajak", "ntpnNtpd", "tanggalBayar", "status", "statusAuto", "keterangan", "sourceData", "sourceSheet", "uploadBatchId", "createdAt", "updatedAt"];
  const esc = (v: unknown) => `"${String(v ?? "").replace(/"/g, '""')}"`;
  return [headers.join(","), ...rows.map((r) => headers.map((h) => esc(h === "pajak" ? r.pajakTerhutang : r[h as keyof TaxTransaction])).join(","))].join("\n");
}
...
export function TaxCoordinatorDashboard() {
  ...
  const [filters, setFilters] = useState<Filters>({ tahun: ALL, masaPajak: ALL, perusahaan: ALL, jenisPajak: ALL, status: ALL, sourceData: ALL, sourceSheet: ALL, search: "" });
  const [message, setMessage] = useState("Belum ada data. Upload Excel atau tambahkan data manual.");
  ...
  async function refreshData() {
    setLoading(true); setError(""); setMessage("Memuat data dari /public/data/tax-data.json...");
    try {
      const [staticRows, staticHistory] = await Promise.all([loadStaticTaxData(), loadStaticUploadHistory()]);
      const manualRows = JSON.parse(localStorage.getItem(MANUAL_STORAGE_KEY) || "[]").map(fromStaticEntry);
      const uploadRows = JSON.parse(localStorage.getItem(UPLOAD_STORAGE_KEY) || "[]").map(fromStaticEntry);
      const sessionHistory = JSON.parse(localStorage.getItem(UPLOAD_HISTORY_STORAGE_KEY) || "[]");
      setRecords([...staticRows, ...uploadRows, ...manualRows]);
      setUploadBatches([...sessionHistory, ...staticHistory]);
      setMessage(staticRows.length || uploadRows.length || manualRows.length ? "Mode file statis aktif. Data utama dibaca dari /public/data/tax-data.json." : "Belum ada data. Silakan upload Excel atau update file data statis.");
    } catch (err) { setError(err instanceof Error ? err.message : "File data statis gagal dibaca."); }
    finally { setLoading(false); }
  }
  useEffect(() => { const saved = localStorage.getItem(FILTER_STORAGE_KEY); if (saved) setFilters((cur) => ({ ...cur, ...JSON.parse(saved) })); refreshData(); }, []);
  ...
  function persistManual(rows: TaxTransaction[]) { localStorage.setItem(MANUAL_STORAGE_KEY, JSON.stringify(rows.map(toStaticEntry))); }
  function persistUploads(rows: TaxTransaction[]) { localStorage.setItem(UPLOAD_STORAGE_KEY, JSON.stringify(rows.map(toStaticEntry))); }
  function saveManual() { const errors = validateManualForm(form); setFormErrors(errors); if (Object.keys(errors).length) return; setBusy(true); try { const next = normalizeManualRecord(form); const manualRows = records.filter((r) => r.sourceData === "Manual Input" && r.id !== next.id); const nextManual = [...manualRows, next]; persistManual(nextManual); setRecords((cur) => [...cur.filter((r) => r.sourceData !== "Manual Input" && r.id !== next.id), ...nextManual]); setMessage("Data manual tersimpan sementara di browser ini. Klik Download Updated JSON untuk membuat file statis baru."); setModalOpen(false); } finally { setBusy(false); } }
  function deleteManual(id: string) { if (!confirm("Apakah Anda yakin ingin menghapus data manual dari browser ini?")) return; const nextManual = records.filter((r) => r.sourceData === "Manual Input" && r.id !== id); persistManual(nextManual); setRecords((cur) => cur.filter((r) => r.id !== id)); }
  function deleteBatch(id: string) { if (!confirm("Hapus data upload Excel sementara dari browser ini?")) return; const nextUploads = records.filter((r) => r.sourceData === "Excel Import" && r.uploadBatchId !== id); const others = records.filter((r) => r.sourceData !== "Excel Import"); persistUploads(nextUploads); setRecords([...others, ...nextUploads]); const nextHistory = uploadBatches.filter((b) => b.id !== id); localStorage.setItem(UPLOAD_HISTORY_STORAGE_KEY, JSON.stringify(nextHistory.filter((b) => !b.id.startsWith("static-")))); setUploadBatches(nextHistory); }
  function exportCsv() { downloadText("tax-dashboard-export.csv", toCsv(filtered), "text/csv;charset=utf-8"); }
  function exportJson(rows = records, filename = "tax-data.json") { downloadText(filename, JSON.stringify(rows.map(toStaticEntry), null, 2), "application/json;charset=utf-8"); }
  function exportUploadHistory() { downloadText("upload-history.json", JSON.stringify(uploadBatches, null, 2), "application/json;charset=utf-8"); }
  function importExcel(event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader();
    reader.onload = () => { setBusy(true); setMessage("Memproses Excel..."); try { const result = parseWorkbook(XLSX.read(reader.result, { type: "array", cellDates: false })); setError(result.errors.join(" ")); const batchId = `upload-${crypto.randomUUID()}`; const now = new Date().toISOString(); const recordsFromUpload = result.records.map((r) => ({ ...r, id: `excel-${crypto.randomUUID()}`, uploadBatchId: batchId, createdAt: now, updatedAt: now })); const shouldMerge = !records.length || confirm("Gabungkan data Excel dengan data yang sudah ada? Pilih Cancel untuk mengganti data upload sementara sebelumnya."); const existingUploads = shouldMerge ? records.filter((r) => r.sourceData === "Excel Import") : []; const nextUploads = [...existingUploads, ...recordsFromUpload]; persistUploads(nextUploads); const nonUploads = records.filter((r) => r.sourceData !== "Excel Import"); setRecords([...nonUploads, ...nextUploads]); const nextBatch = { id: batchId, file_name: file.name, uploaded_at: now, total_rows: recordsFromUpload.length, uploaded_by: "browser-session", status: "session-preview", error_message: result.errors.join(" ") }; const nextHistory = [nextBatch, ...uploadBatches]; localStorage.setItem(UPLOAD_HISTORY_STORAGE_KEY, JSON.stringify(nextHistory)); setUploadBatches(nextHistory); setMessage("Excel berhasil dinormalisasi untuk preview/session browser. Download JSON Data lalu ganti /public/data/tax-data.json untuk menyimpan permanen."); } catch (err) { console.error("[tax-dashboard] Gagal memproses upload Excel", err); setError(err instanceof Error ? err.message : "Data Excel gagal diproses."); } finally { setBusy(false); } };
  ...
  return <main className="min-h-screen bg-[#EEF3F8] text-slate-950">
    ...
    <div className="min-h-screen lg:pl-72">
      ...
      <section className="space-y-6 p-4 sm:p-6 xl:p-8">
        ...
        <FilterBar filters={filters} updateFilter={updateFilter} options={{ tahun: options("tahun"), masaPajak: options("masaPajak"), perusahaan: options("perusahaan"), jenisPajak: TAX_TYPES.filter((type) => !meta.types || meta.types.includes(type)), status: STATUSES, sourceData: ["Static File", "Excel Import", "Manual Input"], sourceSheet: options("sourceSheet") }} onUpload={() => inputRef.current?.click()} onExport={exportCsv} onExportJson={() => exportJson()} onExportHistory={exportUploadHistory} onManual={() => openManual()} />
    ...
    {modalOpen && <ManualModal page={page} form={form} setForm={setForm} errors={formErrors} onClose={() => setModalOpen(false)} onSave={saveManual} saving={busy} onDownload={() => exportJson()} />}
  </main>;
}
...
function FilterBar({ filters, updateFilter, options, onUpload, onExport, onExportJson, onExportHistory, onManual }: { filters: Filters; updateFilter: (key: keyof Filters, value: string) => void; options: { tahun: string[]; masaPajak: string[]; perusahaan: string[]; jenisPajak: readonly string[]; status: readonly string[]; sourceData: string[]; sourceSheet: string[] }; onUpload: () => void; onExport: () => void; onExportJson: () => void; onExportHistory: () => void; onManual: () => void }) {
  const selects: [keyof Filters, string, readonly string[]][] = [["tahun", "Semua Tahun", options.tahun], ["masaPajak", "Semua Masa Pajak", options.masaPajak], ["perusahaan", "Semua Perusahaan", options.perusahaan], ["jenisPajak", "Semua Jenis Pajak", options.jenisPajak], ["status", "Semua Status", options.status], ["sourceData", "Semua Source Data", options.sourceData], ["sourceSheet", "Semua Source Sheet", options.sourceSheet]];
  return <Card className="rounded-3xl border-[#D8E0EA] shadow-sm"><CardContent className="grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-12">{selects.map(([key, placeholder, values]) => <Select key={key} value={filters[key]} onChange={(e) => updateFilter(key, e.target.value)} className="h-11 rounded-2xl bg-white"><option value={ALL}>{placeholder}</option>{values.map((v) => <option key={v} value={v}>{v}</option>)}</Select>)}<div className="relative xl:col-span-1"><Search className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-slate-400" /><Input value={filters.search} onChange={(e) => updateFilter("search", e.target.value)} placeholder="Cari perusahaan, NTPN, jenis pajak..." className="h-11 rounded-2xl bg-white pl-9" /></div><Button onClick={onUpload} className="h-11 rounded-2xl bg-blue-600 font-bold hover:bg-blue-700"><Upload className="h-4 w-4" /> Upload Excel</Button><Button onClick={onManual} variant="outline" className="h-11 rounded-2xl font-bold"><Plus className="h-4 w-4" /> Manual</Button><Button onClick={onExport} variant="outline" className="h-11 rounded-2xl font-bold"><Download className="h-4 w-4" /> Download CSV Data</Button><Button onClick={onExportJson} variant="outline" className="h-11 rounded-2xl font-bold"><Download className="h-4 w-4" /> Download JSON Data</Button><Button onClick={onExportHistory} variant="outline" className="h-11 rounded-2xl font-bold"><Download className="h-4 w-4" /> Download Upload History JSON</Button></CardContent></Card>;
}
