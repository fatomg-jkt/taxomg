...
type Page = "dashboard" | "ppn" | "pph21" | "unifikasi" | "pb1" | "umkm" | "documents" | "updateSaldo";
...
const pageMeta: Record<Page, { title: string; subtitle: string; types?: TaxType[] }> = {
  ...
  updateSaldo: { title: "Update Saldo All Brand", subtitle: "Monitoring saldo bank, mutasi masuk, mutasi keluar, dan saldo akhir seluruh brand." },
};

const navItems = [
  ["dashboard", Home, "Dashboard"], ["ppn", Receipt, "PPN"], ["pph21", Receipt, "PPh Pasal 21"], ["unifikasi", Receipt, "PPh Unifikasi"], ["pb1", Building2, "PB1"], ["umkm", Building2, "PPh UMKM"], ["documents", FileArchive, "Dokumen Pajak"], ["updateSaldo", Landmark, "Update Saldo"],
] as const;
...
type UpdateSaldoRecord = { id: string; tanggal: string; brand: string; perusahaan: string; bank: string; nomorRekening: string; deskripsi: string; uangMasuk: number; uangKeluar: number; saldoAkhir: number; kategori: string; source: "Excel Import" | "Manual Input" | string; keterangan: string; createdAt?: string; updatedAt?: string };
type UpdateSaldoFilters = { tahun: string; bulan: string; brand: string; bank: string; kategori: string };
type UpdateSaldoForm = { id?: string; tanggal: string; brand: string; perusahaan: string; bank: string; nomorRekening: string; deskripsi: string; uangMasuk: string; uangKeluar: string; saldoAkhir: string; kategori: string; keterangan: string };
const UPDATE_SALDO_FILTER_STORAGE_KEY = "update-saldo-filters-v1";
const emptyUpdateSaldoForm = (): UpdateSaldoForm => ({ tanggal: "", brand: "", perusahaan: "", bank: "", nomorRekening: "", deskripsi: "", uangMasuk: "", uangKeluar: "", saldoAkhir: "", kategori: "", keterangan: "" });
...
export function TaxCoordinatorDashboard() {
  ...
  async function saveUpdateSaldoToCloud() {
    ...
    try {
      ...
      setUpdateSaldoLastSaved(payload.updatedAt); setMessage("Save to Cloud Update Saldo berhasil. Data tersimpan terpisah dari data pajak.");
    } catch (err) { setError(err instanceof Error ? err.message : "Save to Cloud Update Saldo gagal."); } finally { setBusy(false); }
  }
  async function saveUpdateSaldoManual() { const password = await verifyPassword(); if (!password) return; setUpdateSaldoRecords((rows) => updateSaldoForm.id ? rows.map((row) => row.id === updateSaldoForm.id ? updateSaldoFromForm(updateSaldoForm) : row) : [...rows, updateSaldoFromForm(updateSaldoForm)]); setUpdateSaldoModalOpen(false); setUpdateSaldoForm(emptyUpdateSaldoForm()); setMessage("Data Update Saldo manual ditambahkan. Klik Save to Cloud agar tersimpan."); }
  async function deleteUpdateSaldo(id: string) { const password = await verifyPassword(); if (!password) return; if (!confirm("Hapus data Update Saldo ini?")) return; setUpdateSaldoRecords((rows) => rows.filter((row) => row.id !== id)); setMessage("Data Update Saldo dihapus dari state. Klik Save to Cloud agar tersimpan."); }
  function importUpdateSaldoExcel(event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0]; if (!file) return;
    const reader = new FileReader();
    reader.onload = async () => { const password = await verifyPassword(); if (!password) { event.target.value = ""; return; } setBusy(true); try { const rows = parseUpdateSaldoWorkbook(reader.result as ArrayBuffer); setUpdateSaldoRecords((current) => [...current, ...rows]); setMessage(rows.length ? `Upload Excel Update Saldo berhasil: ${rows.length} baris.` : "Excel Update Saldo kosong. Tidak ada data yang ditambahkan."); } catch (err) { setError(err instanceof Error ? err.message : "Upload Excel Update Saldo gagal."); } finally { setBusy(false); event.target.value = ""; } };
  ...
  const updateSaldoFiltered = useMemo(() => updateSaldoRecords.filter((r) => { const d = new Date(r.tanggal); const year = Number.isNaN(d.getTime()) ? DEFAULT_DASHBOARD_YEAR : String(d.getFullYear()); return (updateSaldoFilters.tahun === ALL || year === updateSaldoFilters.tahun) && (updateSaldoFilters.bulan === ALL || updateSaldoMonth(r) === monthIndex(updateSaldoFilters.bulan)) && (updateSaldoFilters.brand === ALL || brandName(r) === updateSaldoFilters.brand) && (updateSaldoFilters.bank === ALL || r.bank === updateSaldoFilters.bank) && (updateSaldoFilters.kategori === ALL || r.kategori === updateSaldoFilters.kategori); }), [updateSaldoRecords, updateSaldoFilters]);
  ...
  return <main className="min-h-screen bg-[#EEF3F8] text-slate-950" style={page === "dashboard" ? { fontFamily: PROFESSIONAL_FONT_STACK } : undefined}>
    ...
    <div className="min-h-screen lg:pl-72">
      ...
      <section className="space-y-6 p-4 sm:p-6 xl:p-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><h1 className="text-3xl font-black tracking-tight sm:text-4xl">{meta.title}</h1>{meta.subtitle && <p className="mt-2 text-base font-medium text-slate-600">{meta.subtitle}</p>}</div>{isManualPage(page) && page !== "dashboard" && page !== "updateSaldo" && <Button onClick={() => openManual()} className="rounded-2xl bg-blue-600 font-bold hover:bg-blue-700"><Plus className="h-4 w-4" /> {manualButtonLabel(page)}</Button>}</div>
        {page === "updateSaldo" ? <UpdateSaldoFilterBar filters={updateSaldoFilters} setFilters={setUpdateSaldoFilters} options={updateSaldoOptions} onUpload={() => updateSaldoInputRef.current?.click()} onManual={() => { setUpdateSaldoForm(emptyUpdateSaldoForm()); setUpdateSaldoModalOpen(true); }} onSave={saveUpdateSaldoToCloud} saving={busy} /> : <FilterBar filters={filters} updateFilter={updateFilter} options={{ tahun: yearOptions, masaPajak: MONTH_NAMES, perusahaan: options("perusahaan"), jenisPajak: page === "dashboard" ? DASHBOARD_FILTER_TAX_TYPES : TAX_TYPES.filter((type) => !meta.types || meta.types.includes(type)), status: FILTER_STATUSES }} onUpload={() => inputRef.current?.click()} onManual={() => openManual()} onSave={saveToCloud} saving={busy} showDataEntryActions={page !== "dashboard" && page !== "documents"} showStatusAndSearch={page !== "documents" && page !== "dashboard"} />}
