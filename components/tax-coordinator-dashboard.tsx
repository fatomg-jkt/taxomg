...
const pageMeta: Record<Page, { title: string; subtitle: string; types?: TaxType[] }> = {
  ...
  cashflowPaymentRequest: { title: "Cashflow · Pengajuan Pembayaran", subtitle: "Input dan pengelolaan pengajuan pembayaran." },
...
const cashflowPages: CashflowPage[] = ["cashflow", "cashflowProjection", "cashflowActual", "cashflowPaymentRequest"];
...
function Sidebar({ page, setPage, open, setOpen, user, onLogout }: { page: Page; setPage: (page: Page) => void; open: boolean; setOpen: (open: boolean) => void; user: { email: string; role: UserRole }; onLogout: () => void }) {
  ...
  const [cashflowExpanded, setCashflowExpanded] = useState(() => page === "cashflowProjection" || page === "cashflowActual" || page === "cashflowPaymentRequest");
  ...
  useEffect(() => {
    ...
    if (page === "cashflowProjection" || page === "cashflowActual" || page === "cashflowPaymentRequest") setCashflowExpanded(true);
...
function DashboardOverview({ rows, documentCount }: { rows?: TaxTransaction[]; documentCount?: number }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-6 xl:grid-cols-5"><Card className="overflow-hidden rounded-3xl border-emerald-100 bg-gradient-to-br from-white via-white to-emerald-50/60 shadow-[0_18px_50px_-28px_rgba(16,185,129,0.45)] xl:col-span-2"><CardHeader className="flex-row items-start justify-between space-y-0"><div><CardTitle>Komposisi Pajak</CardTitle><CardDescription>Distribusi nilai pajak berdasarkan jenis utama.</CardDescription></div><Badge className="rounded-full border border-emerald-200 bg-emerald-100 px-3 py-1 text-[10px] font-black uppercase tracking-wide text-emerald-700 hover:bg-emerald-100">{donutData.length} Jenis Pajak</Badge></CardHeader><CardContent className="grid h-80 grid-cols-[minmax(0,1.1fr)_minmax(145px,.9fr)] gap-3 pb-5"><div className="relative min-w-0"><ResponsiveContainer width="100%" height="100%"><PieChart>{donutData.length ? <Pie data={donutData} dataKey="value" nameKey="name" innerRadius={62} outerRadius={94} paddingAngle={4} cornerRadius={8} stroke="#fff" strokeWidth={3}>{donutData.map((entry) => <Cell key={entry.name} fill={DASHBOARD_COLORS[entry.name] || "#64748b"} />)}</Pie> : <Pie data={[{ name: "Belum ada data", value: 1 }]} dataKey="value" innerRadius={62} outerRadius={94}><Cell fill="#e2e8f0" /></Pie>}<Tooltip formatter={(value: number) => rupiah(value)} contentStyle={{borderRadius:16,border:"1px solid #d1fae5",boxShadow:"0 12px 30px rgba(15,23,42,.12)"}} /></PieChart></ResponsiveContainer><div className="pointer-events-none absolute inset-0 grid place-items-center"><div className="text-center"><p className="text-[10px] font-black uppercase tracking-wide text-slate-400">Total Pajak</p><p className="mt-1 text-base font-black text-slate-950">{rupiah(summary.totalTax)}</p><p className="mt-1 text-[10px] font-bold text-emerald-600">100% Pajak</p></div></div></div><div className="my-2 space-y-2 overflow-y-auto pr-1">{donutData.length ? donutData.map((item) => { const percentage = summary.totalTax > 0 ? item.value / summary.totalTax * 100 : 0; return <div key={item.name} className="rounded-2xl border border-slate-100 bg-white/90 p-3 shadow-sm"><div className="flex items-center justify-between gap-2"><p className="truncate text-xs font-black text-slate-800"><span className="mr-2 inline-block h-2.5 w-2.5 rounded-full" style={{backgroundColor:DASHBOARD_COLORS[item.name] || "#64748b"}} />{item.name}</p><span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-black text-slate-700">{percentage.toFixed(1)}%</span></div><div className="mt-2 flex items-center gap-2"><div className="h-1.5 flex-1 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full" style={{width:`${Math.min(percentage,100)}%`,backgroundColor:DASHBOARD_COLORS[item.name] || "#64748b"}} /></div><span className="whitespace-nowrap text-[10px] font-bold text-slate-500">{rupiah(item.value)}</span></div></div>; }) : <p className="grid h-full place-items-center text-xs font-semibold text-slate-400">Belum ada data</p>}</div></CardContent></Card><Card className="overflow-hidden rounded-3xl border-blue-100 bg-gradient-to-br from-white to-blue-50/40 shadow-[0_18px_50px_-28px_rgba(37,99,235,0.45)] xl:col-span-3"><CardHeader className="flex-row items-start justify-between space-y-0"><div><CardTitle>Pajak per Jenis Pajak</CardTitle><CardDescription>Perbandingan pajak terutang dan sudah dibayar.</CardDescription></div><Badge className="rounded-full border border-blue-200 bg-blue-100 px-3 py-1 text-[10px] font-black uppercase tracking-wide text-blue-700 hover:bg-blue-100">Comparison</Badge></CardHeader><CardContent className="h-80 pb-5"><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} barGap={5} margin={{top:8,right:10,left:0,bottom:0}}><defs><linearGradient id="taxTotalGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#2563eb"/><stop offset="100%" stopColor="#60a5fa"/></linearGradient><linearGradient id="taxPaidGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#059669"/><stop offset="100%" stopColor="#34d399"/></linearGradient></defs><CartesianGrid stroke="#dbe4f0" strokeDasharray="4 5" vertical={false}/><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fontSize:11,fill:"#475569",fontWeight:600}} dy={8}/><YAxis axisLine={false} tickLine={false} tick={{fontSize:11,fill:"#64748b"}} tickFormatter={(value: number) => `${Math.round(value / 1000000)} jt`} width={54}/><Tooltip cursor={{fill:"#eff6ff",radius:12}} contentStyle={{borderRadius:16,border:"1px solid #dbeafe",boxShadow:"0 12px 30px rgba(15,23,42,.12)"}} formatter={(value: number) => rupiah(value)}/><Legend iconType="circle" wrapperStyle={{paddingTop:14,fontWeight:600}}/><Bar dataKey="value" name="Total Pajak" radius={[10,10,3,3]} fill="url(#taxTotalGradient)" maxBarSize={42}/><Bar dataKey="paid" name="Sudah Dibayar" radius={[10,10,3,3]} fill="url(#taxPaidGradient)" maxBarSize={42}/></BarChart></ResponsiveContainer></CardContent></Card></section>
...
function FinanceOverview({ accounts }: { accounts: FinanceAccount[] }) {
  ...
  const groupedBalances = (key: "entity" | "provider") => {
    const groupName = (account: FinanceAccount) => key === "entity" && account.entity === "Entity Belum Diisi" ? account.brand : account[key].trim();
    return Array.from(new Set(selectedAccounts.map(groupName).filter(Boolean))).map((name) => ({ name, total: selectedAccounts.filter((account) => groupName(account) === name).reduce((sum, account) => sum + account.balance, 0), count: selectedAccounts.filter((account) => groupName(account) === name).length }));
  };
    ...
    <section>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {summary.map((brand) => <Card
          ...
          className={cn("cursor-pointer overflow-hidden rounded-3xl border-0 text-white shadow-sm transition duration-200 hover:-translate-y-1 hover:shadow-xl focus:outline-none focus:ring-4 focus:ring-blue-300", selectedBrand === brand.brand && "md:col-span-2 xl:col-span-3 ring-4 ring-blue-300 shadow-xl")}
        >
          <CardContent className="p-6">
            ...
            <div className="mt-6 flex items-center justify-between gap-3 text-sm font-bold text-white/80"><span>{plainNumber(brand.accountCount)} rekening / account</span><span className="text-xs">{selectedBrand === brand.brand ? "Tutup detail" : "Lihat detail"}</span></div>
          </CardContent>
          {selectedBrand === brand.brand && <div className="border-t border-white/20 bg-white p-5 text-slate-900" onClick={(event)=>event.stopPropagation()} onKeyDown={(event)=>event.stopPropagation()}>{selectedAccounts.length ? <div><div className="mb-4"><h4 className="text-lg font-black">Rincian Saldo {brand.brand}</h4><p className="text-sm font-medium text-slate-500">Saldo per PT/Entity dan Bank/Provider.</p></div><div className="grid gap-5 lg:grid-cols-2"><EntityBalanceGroup rows={entityBalances} accounts={selectedAccounts}/><BalanceGroup title="Saldo per Bank / Provider" empty="Bank / Provider belum diisi." rows={providerBalances}/></div></div> : <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center text-sm font-semibold text-slate-500">Belum ada data saldo untuk brand {brand.brand}.</div>}</div>}
        </Card>)}
      </div>
    </section>
...
function EntityBalanceGroup({ rows, accounts }: { rows: { name: string; total: number; count: number }[]; accounts: FinanceAccount[] }) {
  const [openEntity, setOpenEntity] = useState("");
  return <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4"><h3 className="mb-3 font-black text-slate-900">Saldo per PT / Entity</h3><div className="space-y-2">{rows.length ? rows.map((row)=>{const entityAccounts=accounts.filter((account)=>account.entity===row.name||(account.entity==="Entity Belum Diisi"&&account.brand===row.name)); const expanded=openEntity===row.name; return <div key={row.name} className="overflow-hidden rounded-xl bg-white shadow-sm"><button type="button" onClick={()=>setOpenEntity((current)=>current===row.name?"":row.name)} className="flex w-full items-center justify-between gap-4 p-3 text-left transition hover:bg-blue-50" aria-expanded={expanded}><div className="min-w-0"><p className="truncate text-sm font-bold text-slate-800">{row.name}</p><p className="text-xs font-medium text-slate-500">{plainNumber(row.count)} rekening / account</p></div><div className="flex shrink-0 items-center gap-3"><p className="font-black text-slate-950">{rupiah(row.total)}</p><ChevronDown className={cn("h-4 w-4 text-slate-500 transition-transform",expanded&&"rotate-180")}/></div></button>{expanded&&<div className="border-t border-slate-100 bg-blue-50/40 p-3"><p className="mb-2 text-[11px] font-black uppercase tracking-wider text-blue-700">Rincian rekening {row.name}</p><div className="space-y-2">{entityAccounts.map((account)=><div key={account.id} className="grid gap-2 rounded-xl border border-blue-100 bg-white p-3 sm:grid-cols-[minmax(0,0.8fr)_minmax(0,1fr)_minmax(10rem,0.75fr)] sm:items-center"><div><p className="text-sm font-bold text-slate-800">{account.provider||"Bank / Provider belum diisi"}</p><p className="text-xs text-slate-500">{account.accountType||"Tipe belum diisi"}</p></div><div><p className="text-sm font-semibold text-slate-700">{account.accountName||"Nama rekening belum diisi"}</p><p className="text-xs text-slate-500">{account.accountNumber||"Nomor / kode belum diisi"}</p></div><p className="text-right font-black text-slate-950">{rupiah(account.balance)}</p></div>)}</div></div>}</div>}) : <p className="rounded-xl border border-dashed border-slate-300 p-5 text-center text-sm font-semibold text-slate-500">PT / Entity belum diisi.</p>}</div></div>;
}
...
function brandHeaderStyle(brand: string) {
  const colors: Record<string, string> = { "1001": "#EC4899", "MAISON Y": "#C026D3", Obsidian: "#111827", PADEL: "#F59E0B", GOSE: "#7C3AED", BAC: "#1687D9", OMG: "#64748B", "PT GLOBAL SEHAT BERKARYA": "#334155", "TRIPLE EGG": "#10B981", WOK: "#DC2626", HUNIAN: "#FDE68A", "PT SEBELUM HINGGA SESUDAH": "#047857", Resto: "#059669" };
  return colors[brand] ? { background: colors[brand], color: brand === "HUNIAN" ? "#0F172A" : "white" } : undefined;
}
  ...
  return <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{brands.map((brand)=>{
    ...
    return <Card key={brand} className={cn("overflow-hidden rounded-3xl border-[#D8E0EA] bg-white shadow-sm transition-shadow", expanded && "md:col-span-2 xl:col-span-3 shadow-xl ring-2 ring-blue-300")}>
      ...
      {expanded && <CardContent className="space-y-4 p-5">
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="mb-3 flex flex-wrap items-center justify-between gap-3"><div><p className="text-[11px] font-black uppercase tracking-widest text-slate-400">{structure.length ? "Rekening lain / belum terpetakan" : "Rekening Brand"}</p><h3 className="font-black text-slate-800">{structure.length ? `Entity Default ${brand}` : brand}</h3></div><Button onClick={() => onAddAccount(brand)} size="sm" variant="outline" className="rounded-xl font-bold"><Plus className="h-3.5 w-3.5"/> Tambah Rekening</Button></div>{legacyRows.length ? <FinanceAccountRows rows={legacyRows} onUpdate={onUpdateAccount} onDelete={onDeleteAccount}/> : <p className="rounded-xl border border-dashed border-slate-200 p-4 text-sm font-semibold text-slate-400">Belum ada rekening tambahan untuk brand ini.</p>}</div>
      </CardContent>}
    </Card>})}</div>;
}
