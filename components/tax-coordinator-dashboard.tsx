...
const MANUAL_TAX_TYPES = ["PPN Keluaran", "PPN Masukan", "Pembayaran PPN", "PPh Pasal 21", "PPh Pasal 23", "PPh Final 4(2)", "PB1", "PPh UMKM"] as const;
const SOURCE_SHEETS = ["PPH-Resto", "PPN-1001", "PPH-1001", "PPH-OBS"] as const;
...
type ManualForm = { id?: string; sourceSheet: string; perusahaan: string; tahun: string; masaPajak: string; jenisPajak: TaxType; dpp: string; pajak: string; ntpnNtpd: string; tanggalBayar: string; status: string; keterangan: string; ppnKeluaran: string; ppnMasukan: string; pmTidakDikreditkan: string; totalPembayaranPpn: string };
const emptyManualForm = (page: Page): ManualForm => ({ id: undefined, sourceSheet: SOURCE_SHEETS[0], perusahaan: "", tahun: String(new Date().getFullYear()), masaPajak: "", jenisPajak: page === "pb1" ? "PB1" : page === "ppn" ? "PPN" : page === "umkm" ? "PPh UMKM" : page === "unifikasi" ? "PPh Pasal 23" : "PPh Pasal 21", dpp: "", pajak: "", ntpnNtpd: "", tanggalBayar: "", status: "", keterangan: "", ppnKeluaran: "", ppnMasukan: "", pmTidakDikreditkan: "", totalPembayaranPpn: "" });
...
function normalizeStaticEntry(row: StaticTaxEntry, index: number): TaxTransaction {
  const pajak = row.pajak ?? row.pajakTerhutang ?? row.pajakTerutang ?? 0;
  ...
  return { id: clean(row.id) || `static-${index + 1}`, perusahaan: clean(row.perusahaan ?? row.company) || "Perusahaan Belum Diisi", tahun: clean(row.tahun) || periodYear(clean(row.masaPajak ?? row.masa_pajak ?? row.masa)), masaPajak: clean(row.masaPajak ?? row.masa_pajak ?? row.masa) || "-", jenisPajak: (row.jenisPajak ?? row.jenis_pajak ?? "PPh Pasal 21") as TaxType, dpp: dppValue, pajakTerhutang: pajakValue, ntpnNtpd, tanggalBayar: clean(row.tanggalBayar ?? row.tanggal_bayar), status: clean(row.status) || displayStatus(statusAuto), statusAuto, keterangan: clean(row.keterangan), sourceData: row.sourceData ?? row.source_data ?? (row.source as TaxTransaction["sourceData"]) ?? "Static File", sourceSheet: clean(row.sourceSheet ?? row.source_sheet) || "tax-data.json", sourceRow: Number(row.sourceRow ?? row.source_row) || index + 1, uploadBatchId: row.uploadBatchId ?? row.upload_batch_id, createdAt: clean(row.createdAt ?? row.created_at), updatedAt: clean(row.updatedAt ?? row.updated_at) };
}
function toStaticEntry(row: TaxTransaction) {
  return { id: row.id, sourceSheet: row.sourceSheet, company: row.perusahaan, perusahaan: row.perusahaan, masa: row.masaPajak, masaPajak: row.masaPajak, tahun: row.tahun, jenisPajak: row.jenisPajak, dpp: row.dpp, pajakTerutang: row.pajakTerhutang, pajak: row.pajakTerhutang, ntpnNtpd: row.ntpnNtpd, tanggalBayar: row.tanggalBayar || "", status: row.status, statusAuto: row.statusAuto || "", source: row.sourceData || "Static File", sourceData: row.sourceData || "Static File", keterangan: row.keterangan, createdAt: row.createdAt || new Date().toISOString(), updatedAt: row.updatedAt || new Date().toISOString() };
}
...
export function TaxCoordinatorDashboard() {
  ...
  const [message, setMessage] = useState("Data utama dibaca dari Vercel Blob.");
  ...
  async function refreshData() {
    setLoading(true); setError(""); setMessage("Memuat data pajak dari cloud...");
    const historyPromise = loadStaticUploadHistory();
    try {
      const response = await fetch("/api/tax-data", { cache: "no-store" });
      if (!response.ok) throw new Error("Cloud data tidak bisa dibaca.");
      const payload = await response.json() as { records?: StaticTaxEntry[]; updatedAt?: string | null };
      const cloudRecords = Array.isArray(payload.records) ? payload.records.map(normalizeStaticEntry) : [];
      setRecords(cloudRecords); setLastSaved(payload.updatedAt ?? null);
      setMessage(cloudRecords.length ? "Data cloud berhasil dimuat dari Vercel Blob." : "Belum ada data cloud. Silakan tambah data manual atau upload Excel, lalu Save to Cloud.");
    } catch (err) {
      console.error("[tax-dashboard] Gagal memuat cloud data", err);
      const fallback = localStorage.getItem("tax-dashboard-records-fallback-v1");
      setRecords(fallback ? JSON.parse(fallback).map(normalizeStaticEntry) : []);
      setMessage("Blob read gagal; dashboard tetap berjalan dengan fallback lokal/kosong.");
    } finally {
      setUploadBatches(await historyPromise);
      setLoading(false);
    }
  }
  useEffect(() => { const saved = localStorage.getItem(FILTER_STORAGE_KEY); if (saved) { const parsed = JSON.parse(saved) as Partial<Filters>; setFilters({ tahun: parsed.tahun ?? ALL, masaPajak: parsed.masaPajak ?? ALL, perusahaan: parsed.perusahaan ?? ALL, jenisPajak: parsed.jenisPajak ?? ALL, status: parsed.status ?? ALL, search: parsed.search ?? "" }); } refreshData(); }, []);
  useEffect(() => localStorage.setItem(FILTER_STORAGE_KEY, JSON.stringify(filters)), [filters]);
  useEffect(() => localStorage.setItem("tax-dashboard-records-fallback-v1", JSON.stringify(records.map(toStaticEntry))), [records]);
  ...
  async function saveToCloud() {
    ...
    try {
      const response = await fetch("/api/tax-data", {
        ...
        body: JSON.stringify({ records: records.map(toStaticEntry) }),
      });
      const payload = await response.json().catch(() => ({})) as { ok?: boolean; error?: string; updatedAt?: string };
      if (!response.ok || !payload.ok) throw new Error(payload.error || "Gagal menyimpan data");
      setLastSaved(payload.updatedAt ?? new Date().toISOString());
      setMessage("Data berhasil disimpan ke cloud.");
      alert("Data berhasil disimpan ke cloud");
    } catch (err) {
      console.error("[tax-dashboard] Gagal menyimpan cloud data", err);
  ...
  function openManual(entry?: TaxTransaction) { setFormErrors({}); setForm(entry ? { ...emptyManualForm(page), id: entry.id, sourceSheet: entry.sourceSheet || SOURCE_SHEETS[0], perusahaan: entry.perusahaan, tahun: entry.tahun, masaPajak: entry.masaPajak, jenisPajak: entry.jenisPajak, dpp: String(entry.dpp), pajak: String(entry.pajakTerhutang), ntpnNtpd: entry.ntpnNtpd, tanggalBayar: entry.tanggalBayar || "", status: entry.status, keterangan: entry.keterangan, ppnKeluaran: entry.jenisPajak === "PPN" ? String(entry.dpp) : "", ppnMasukan: "", pmTidakDikreditkan: "", totalPembayaranPpn: entry.jenisPajak === "PPN" ? String(entry.pajakTerhutang) : "" } : emptyManualForm(page)); setModalOpen(true); }
  function saveManual() { const errors = validateManualForm(form); setFormErrors(errors); if (Object.keys(errors).length) return; setBusy(true); const next = normalizeManualRecord(form); setRecords((rows) => form.id ? rows.map((row) => row.id === form.id ? next : row) : [...rows, next]); setMessage("Data berubah di dashboard. Klik Save to Cloud agar tersimpan untuk semua user."); setModalOpen(false); setBusy(false); }
  function deleteManual(id: string) { if (!confirm("Apakah Anda yakin ingin menghapus record ini?")) return; setRecords((rows) => rows.filter((row) => row.id !== id)); setMessage("Record dihapus. Klik Save to Cloud agar tersimpan untuk semua user."); }
  function deleteBatch(id: string) { if (!confirm("Hapus riwayat dan data upload sementara ini dari browser?")) return; setUploadBatches((rows) => rows.filter((row) => row.id !== id)); setRecords((rows) => rows.filter((row) => row.uploadBatchId !== id)); }
  function importExcel(event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader();
    reader.onload = async () => { setBusy(true); setMessage("Memproses Excel..."); try { const result = parseWorkbook(XLSX.read(reader.result, { type: "array", cellDates: false })); setError(result.errors.join(" ")); const batchId = `upload-${crypto.randomUUID()}`; const rows = result.records.map((row) => ({ ...row, id: `${batchId}-${row.id}`, uploadBatchId: batchId, sourceData: "Excel Import" as const, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() })); const duplicate = uploadBatches.find((b) => b.file_name === file.name && b.total_rows === result.records.length); if (duplicate && !confirm("File dengan nama dan jumlah baris yang sama pernah diupload di sesi/file statis. Tetap tampilkan sebagai batch baru?")) return; setRecords((current) => confirm("Gabungkan data Excel dengan data yang sedang tampil?") ? [...current, ...rows] : rows); setUploadBatches((current) => [{ id: batchId, file_name: file.name, uploaded_at: new Date().toISOString(), total_rows: rows.length, uploaded_by: "browser-session", status: "success", error_message: result.errors.join(" ") }, ...current]); setMessage("Excel berhasil dinormalisasi. Klik Save to Cloud agar tersimpan untuk semua user."); } catch (err) { console.error("[tax-dashboard] Gagal memproses upload Excel", err); setError(err instanceof Error ? err.message : "Data Excel gagal diproses."); } finally { setBusy(false); } };
  ...
  return <main className="min-h-screen bg-[#EEF3F8] text-slate-950">
    ...
    <div className="min-h-screen lg:pl-72">
      ...
      <section className="space-y-6 p-4 sm:p-6 xl:p-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><h1 className="text-3xl font-black tracking-tight sm:text-4xl">{meta.title}</h1><p className="mt-2 text-base font-medium text-slate-600">{meta.subtitle}</p></div>{isManualPage(page) && <div className="flex flex-wrap gap-3"><Button onClick={saveToCloud} variant="outline" className="rounded-2xl font-bold">Save to Cloud</Button><Button onClick={() => openManual()} className="rounded-2xl bg-blue-600 font-bold hover:bg-blue-700"><Plus className="h-4 w-4" /> {manualButtonLabel(page)}</Button></div>}</div>
        ...
        <div className="rounded-2xl border border-blue-100 bg-white p-4 text-sm font-semibold text-slate-700 shadow-sm"><FileSpreadsheet className="mr-2 inline h-4 w-4 text-blue-600" />{loading ? "Memuat data pajak..." : message}{!records.length && !loading && " KPI akan menampilkan 0 sampai data tersedia."}<div className="mt-2 text-xs text-slate-500">Last saved: {lastSaved ? new Date(lastSaved).toLocaleString("id-ID") : "-"}</div></div>
...
function ManualModal({ page, form, setForm, errors, onClose, onSave, saving }: { page: Page; form: ManualForm; setForm: (form: ManualForm) => void; errors: Record<string, string>; onClose: () => void; onSave: () => void; saving: boolean }) {
  ...
  const taxOptions = MANUAL_TAX_TYPES;
