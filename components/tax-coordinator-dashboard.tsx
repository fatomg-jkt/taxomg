...
export function TaxCoordinatorDashboard() {
  ...
  async function saveManual() { const errors = validateManualForm(form); setFormErrors(errors); if (Object.keys(errors).length) return; setBusy(true); try { const next = normalizeManualRecord(form); if (form.id) await apiJson(`/api/tax-entries/${form.id}`, { method: "PUT", body: JSON.stringify(toApiEntry(next)) }); else await apiJson("/api/tax-entries", { method: "POST", body: JSON.stringify(toApiEntry(next)) }); setMessage("Data manual berhasil disimpan ke database."); setModalOpen(false); await refreshData(); } catch (err) { setError(err instanceof Error ? err.message : "Data gagal disimpan ke database."); } finally { setBusy(false); } }
