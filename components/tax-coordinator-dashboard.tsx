...
export function TaxCoordinatorDashboard() {
  ...
  useEffect(() => { const saved = localStorage.getItem(FILTER_STORAGE_KEY); if (saved) setFilters((cur) => ({ ...cur, ...JSON.parse(saved) })); const manual = localStorage.getItem("tax-dashboard-manual-records-v1"); if (manual) setManualRecords(JSON.parse(manual).map(normalizeStaticEntry)); refreshData(); }, []);
...
function FilterBar({ filters, updateFilter, options, onUpload, onExport, onManual, onExportJson, onExportHistory }: { filters: Filters; updateFilter: (key: keyof Filters, value: string) => void; options: { tahun: string[]; masaPajak: string[]; perusahaan: string[]; jenisPajak: readonly string[]; status: readonly string[]; sourceData: string[]; sourceSheet: string[] }; onUpload: () => void; onExport: () => void; onManual: () => void; onExportJson: () => void; onExportHistory: () => void }) {
  ...
  return <Card className="rounded-3xl border-[#D8E0EA] shadow-sm"><CardContent className="grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-12">{selects.map(([key, placeholder, values]) => <Select key={key} value={filters[key]} onChange={(e) => updateFilter(key, e.target.value)} className="h-11 rounded-2xl bg-white"><option value={ALL}>{placeholder}</option>{values.map((v) => <option key={v} value={v}>{v}</option>)}</Select>)}<div className="relative xl:col-span-1"><Search className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-slate-400" /><Input value={filters.search} onChange={(e) => updateFilter("search", e.target.value)} placeholder="Cari perusahaan, NTPN, jenis pajak..." className="h-11 rounded-2xl bg-white pl-9" /></div><Button onClick={onUpload} className="h-11 rounded-2xl bg-blue-600 font-bold hover:bg-blue-700"><Upload className="h-4 w-4" /> Upload Excel</Button><Button onClick={onManual} variant="outline" className="h-11 rounded-2xl font-bold"><Plus className="h-4 w-4" /> Manual</Button><Button onClick={onExport} variant="outline" className="h-11 rounded-2xl font-bold"><Download className="h-4 w-4" /> Download CSV Data</Button><Button onClick={onExportJson} variant="outline" className="h-11 rounded-2xl font-bold"><Download className="h-4 w-4" /> Download Updated JSON</Button><Button onClick={onExportHistory} variant="outline" className="h-11 rounded-2xl font-bold"><Download className="h-4 w-4" /> Download Upload History JSON</Button></CardContent></Card>;
}
