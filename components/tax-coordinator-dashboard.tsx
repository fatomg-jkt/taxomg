...
type TaxTransaction = {
  ...
  statusAuto?: string;
  keterangan: string;
  sourceData?: "Excel Import" | "Manual Input";
  sourceSheet: string;
  sourceRow: number;
...
type ApiTaxEntry = { id: string; perusahaan: string; tahun: string; masa_pajak: string; jenis_pajak: TaxType; dpp: number | string; pajak: number | string; ntpn_ntpd?: string; tanggal_bayar?: string | null; status: string; status_auto?: string; keterangan?: string; source_data: "Excel Import" | "Manual Input"; source_sheet: string; source_row?: number; upload_batch_id?: string | null; created_at?: string; updated_at?: string };
...
function fromApiEntry(row: ApiTaxEntry): TaxTransaction {
  return { id: row.id, perusahaan: row.perusahaan, tahun: row.tahun, masaPajak: row.masa_pajak, jenisPajak: row.jenis_pajak, dpp: Number(row.dpp) || 0, pajakTerhutang: Number(row.pajak) || 0, ntpnNtpd: row.ntpn_ntpd || "", tanggalBayar: row.tanggal_bayar || "", status: row.status, statusAuto: row.status_auto || "", keterangan: row.keterangan || "", sourceData: row.source_data, sourceSheet: row.source_sheet, sourceRow: Number(row.source_row) || 0, uploadBatchId: row.upload_batch_id, createdAt: row.created_at, updatedAt: row.updated_at };
}
function toApiEntry(row: TaxTransaction) {
  return { perusahaan: row.perusahaan, tahun: row.tahun, masa_pajak: row.masaPajak, jenis_pajak: row.jenisPajak, dpp: row.dpp, pajak: row.pajakTerhutang, ntpn_ntpd: row.ntpnNtpd, tanggal_bayar: row.tanggalBayar || null, status: row.status, status_auto: row.statusAuto || "", keterangan: row.keterangan, source_data: row.sourceData || "Excel Import", source_sheet: row.sourceSheet, source_row: row.sourceRow };
}
async function apiJson<T>(url: string, init?: RequestInit): Promise<T> {
  const response = await fetch(url, { ...init, headers: { "Content-Type": "application/json", ...(init?.headers || {}) } });
  const payload = await response.json().catch(() => ({}));
  console.log("[tax-dashboard] API response", { url, status: response.status, ok: response.ok, payload });
  if (!response.ok) throw new Error(payload.error || "Koneksi database gagal.");
  return payload as T;
}
...
export function TaxCoordinatorDashboard() {
  ...
  async function refreshData() {
    setLoading(true); setError(""); setMessage("Memuat data pajak...");
    try { const data = await apiJson<{ tax_entries: ApiTaxEntry[]; upload_batches: UploadBatch[] }>("/api/tax-entries"); setRecords(data.tax_entries.map(fromApiEntry)); setUploadBatches(data.upload_batches); setMessage(data.tax_entries.length ? "Dashboard berhasil diperbarui dari database." : "Belum ada data. Upload Excel atau tambahkan data manual."); }
    catch (err) { setError(err instanceof Error ? err.message : "Koneksi database gagal."); }
  ...
  async function saveManual() { const errors = validateManualForm(form); setFormErrors(errors); if (Object.keys(errors).length) return; setBusy(true); try { const next = normalizeManualRecord(form); console.log("[tax-dashboard] Data manual yang akan disimpan", toApiEntry(next)); if (form.id) await apiJson(`/api/tax-entries/${form.id}`, { method: "PUT", body: JSON.stringify(toApiEntry(next)) }); else await apiJson("/api/tax-entries", { method: "POST", body: JSON.stringify(toApiEntry(next)) }); setMessage("Data berhasil disimpan."); setModalOpen(false); await refreshData(); } catch (err) { console.error("[tax-dashboard] Gagal menyimpan data manual", err); setError(err instanceof Error ? err.message : "Data gagal disimpan ke database."); } finally { setBusy(false); } }
  async function deleteManual(id: string) { if (!confirm("Apakah Anda yakin ingin menghapus data ini?")) return; setBusy(true); try { await apiJson(`/api/tax-entries/${id}`, { method: "DELETE" }); await refreshData(); } catch (err) { setError(err instanceof Error ? err.message : "Data gagal dihapus dari database."); } finally { setBusy(false); } }
  async function deleteBatch(id: string) { if (!confirm("Hapus semua data dari upload Excel ini?")) return; setBusy(true); try { await apiJson(`/api/upload-batches/${id}`, { method: "DELETE" }); await refreshData(); } catch (err) { setError(err instanceof Error ? err.message : "Data upload gagal dihapus."); } finally { setBusy(false); } }
  function exportCsv() { const blob = new Blob([toCsv(filtered)], { type: "text/csv;charset=utf-8" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "tax-dashboard-export.csv"; a.click(); URL.revokeObjectURL(url); }
  function importExcel(event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader();
    reader.onload = async () => { setBusy(true); setMessage("Memproses Excel..."); try { const result = parseWorkbook(XLSX.read(reader.result, { type: "array", cellDates: false })); setError(result.errors.join(" ")); const duplicate = uploadBatches.find((b) => b.file_name === file.name && b.total_rows === result.records.length); if (duplicate && !confirm("File dengan nama dan jumlah baris yang sama pernah diupload. Tetap upload sebagai batch baru? Pilih Cancel untuk membatalkan.")) return; setMessage("Menyimpan data ke database..."); console.log("[tax-dashboard] Data upload yang akan disimpan", { fileName: file.name, rows: result.records.map(toApiEntry) }); await apiJson("/api/upload-batches", { method: "POST", body: JSON.stringify({ batch: { file_name: file.name, total_rows: result.records.length, uploaded_by: "dashboard-user", status: "success", error_message: result.errors.join(" ") }, entries: result.records.map(toApiEntry) }) }); setMessage("Dashboard berhasil diperbarui."); await refreshData(); } catch (err) { console.error("[tax-dashboard] Gagal menyimpan upload Excel", err); setError(err instanceof Error ? err.message : "Data berhasil diparse tetapi gagal tersimpan."); } finally { setBusy(false); } };
  ...
  return <main className="min-h-screen bg-[#EEF3F8] text-slate-950">
    ...
    <div className="min-h-screen lg:pl-72">
      ...
      <section className="space-y-6 p-4 sm:p-6 xl:p-8">
        ...
        <FilterBar filters={filters} updateFilter={updateFilter} options={{ tahun: options("tahun"), masaPajak: options("masaPajak"), perusahaan: options("perusahaan"), jenisPajak: TAX_TYPES.filter((type) => !meta.types || meta.types.includes(type)), status: STATUSES, sourceData: ["Excel Import", "Manual Input"], sourceSheet: options("sourceSheet") }} onUpload={() => inputRef.current?.click()} onExport={exportCsv} onManual={() => openManual()} />
...
function FilterBar({ filters, updateFilter, options, onUpload, onExport, onManual }: { filters: Filters; updateFilter: (key: keyof Filters, value: string) => void; options: { tahun: string[]; masaPajak: string[]; perusahaan: string[]; jenisPajak: readonly string[]; status: readonly string[]; sourceData: string[]; sourceSheet: string[] }; onUpload: () => void; onExport: () => void; onManual: () => void }) {
  ...
  return <Card className="rounded-3xl border-[#D8E0EA] shadow-sm"><CardContent className="grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-11">{selects.map(([key, placeholder, values]) => <Select key={key} value={filters[key]} onChange={(e) => updateFilter(key, e.target.value)} className="h-11 rounded-2xl bg-white"><option value={ALL}>{placeholder}</option>{values.map((v) => <option key={v} value={v}>{v}</option>)}</Select>)}<div className="relative xl:col-span-1"><Search className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-slate-400" /><Input value={filters.search} onChange={(e) => updateFilter("search", e.target.value)} placeholder="Cari perusahaan, NTPN, jenis pajak..." className="h-11 rounded-2xl bg-white pl-9" /></div><Button onClick={onUpload} className="h-11 rounded-2xl bg-blue-600 font-bold hover:bg-blue-700"><Upload className="h-4 w-4" /> Upload Excel</Button><Button onClick={onManual} variant="outline" className="h-11 rounded-2xl font-bold"><Plus className="h-4 w-4" /> Manual</Button><Button onClick={onExport} variant="outline" className="h-11 rounded-2xl font-bold"><Download className="h-4 w-4" /> Export CSV</Button></CardContent></Card>;
}
