...
import { Building2, Cloud, Download, Edit3, FileText, Plus, Trash2, Upload, X } from "lucide-react";
...
const profileEmpty = (): ProfileDraft => ({ id: "", companyName: "", brandGroup: "", businessType: "", npwp: "", nib: "", address: "", status: "Aktif", notes: "" });
...
export function LegalDocumentDashboard({ page, verifyPassword }: { page: LegalPage; verifyPassword: VerifyPassword }) {
  ...
  return <section className="space-y-6 p-4 sm:p-6 xl:p-8">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-3xl font-black tracking-tight sm:text-4xl">{title}</h1><p className="mt-2 font-medium text-slate-600">{subtitle}</p></div><div className="flex gap-2"><Button variant="outline" disabled={saving} onClick={() => save()} className="rounded-2xl font-bold"><Cloud className="h-4 w-4" />{saving ? "Menyimpan..." : "Save to Cloud"}</Button><Button onClick={() => page === "legalCompany" ? setProfile(profileEmpty()) : page === "legalStructure" ? setStructure(structureEmpty()) : setDocumentDraft(documentEmpty())} className="rounded-2xl bg-blue-600 font-bold hover:bg-blue-700">{page === "legalDocuments" ? <Upload className="h-4 w-4" /> : <Plus className="h-4 w-4" />}{page === "legalCompany" ? "Tambah Company Profile" : page === "legalStructure" ? "Tambah Corporate Structure" : "Upload Document"}</Button></div></div>
...
function CompanyContent({ rows, onEdit, onDelete }: { rows: CompanyProfile[]; onEdit: (v: ProfileDraft) => void; onDelete: (id:string)=>void }) { return <><div className="grid gap-4 sm:grid-cols-3"><Summary icon={Building2} label="Total Perusahaan" value={rows.length}/><Summary icon={Building2} label="Perusahaan Aktif" value={rows.filter(r=>r.status==="Aktif").length}/><Summary icon={FileText} label="Perlu Update" value={rows.filter(r=>r.status==="Perlu Update").length}/></div><DataCard title="Daftar Perusahaan">{!rows.length ? <Empty text="Belum ada Company Profile. Klik Tambah Company Profile untuk mulai."/> : <Table><TableHeader><TableRow>{["Nama Perusahaan","Brand / Group","Bentuk Badan Usaha","NPWP","NIB","Alamat","Status","Keterangan","Aksi"].map(h=><TableHead key={h} className="whitespace-nowrap">{h}</TableHead>)}</TableRow></TableHeader><TableBody>{rows.map(r=><TableRow key={r.id}><TableCell className="font-bold">{r.companyName}</TableCell><TableCell>{r.brandGroup||"-"}</TableCell><TableCell>{r.businessType||"-"}</TableCell><TableCell>{r.npwp||"-"}</TableCell><TableCell>{r.nib||"-"}</TableCell><TableCell className="min-w-56">{r.address||"-"}</TableCell><TableCell><StatusBadge value={r.status}/></TableCell><TableCell>{r.notes||"-"}</TableCell><TableCell><Actions edit={()=>onEdit(r)} remove={()=>onDelete(r.id)}/></TableCell></TableRow>)}</TableBody></Table>}</DataCard></>; }
...
function ProfileModal({value,setValue,onSubmit,saving}:{value:ProfileDraft;setValue:(v:ProfileDraft|null)=>void;onSubmit:(e:FormEvent)=>void;saving:boolean}){const set=(k:keyof ProfileDraft,v:string)=>setValue({...value,[k]:v});return <Modal title={value.id?"Edit Company Profile":"Tambah Company Profile"} onClose={()=>setValue(null)} onSubmit={onSubmit} saving={saving}>{[["Nama Perusahaan","companyName"],["Brand / Group","brandGroup"],["Bentuk Badan Usaha","businessType"],["NPWP","npwp"],["NIB","nib"]].map(([l,k])=><Field key={k} label={l}><Input required={k==="companyName"} value={String(value[k as keyof ProfileDraft])} onChange={e=>set(k as keyof ProfileDraft,e.target.value)} className="rounded-xl"/></Field>)}<Field label="Status"><Select value={value.status} onChange={e=>set("status",e.target.value)}>{COMPANY_STATUSES.map(v=><option key={v}>{v}</option>)}</Select></Field><Field label="Alamat" wide><textarea value={value.address} onChange={e=>set("address",e.target.value)} className="min-h-24 w-full rounded-xl border p-3"/></Field><Field label="Keterangan" wide><textarea value={value.notes} onChange={e=>set("notes",e.target.value)} className="min-h-24 w-full rounded-xl border p-3"/></Field></Modal>}
