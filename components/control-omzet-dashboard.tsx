        ...
        <div className="overflow-x-auto bg-slate-100/70 p-3">
          <table className="w-max min-w-full border-separate border-spacing-y-1 font-sans text-xs text-slate-700">
            <thead className="sticky top-0 z-20">
              ...
              <tr>{visibleGroups.flatMap((group) => group.entities.map((entity) => <th key={`${group.name}-${entity}`} colSpan={isObsidianUnreported(group.name, entity) ? 1 : 2} rowSpan={isObsidianUnreported(group.name, entity) ? 2 : 1} className={`${isObsidianUnreported(group.name, entity) ? "w-32 min-w-32 max-w-32 whitespace-normal leading-tight" : "min-w-52"} border-b border-r border-slate-300 px-3 py-3 text-center text-sm font-extrabold shadow-inner ${GROUP_COLORS[group.name].entity}`}>{isObsidianUnreported(group.name, entity) ? <span className="block">Omset Tidak<br />Terlapor<br />PGO + STB</span> : entity}</th>))}</tr>
              ...
              <tr className="bg-slate-100 font-semibold italic text-slate-800">
                ...
                {visibleGroups.flatMap((group) => group.entities.flatMap((entity) => {
                  if (isObsidianUnreported(group.name, entity)) {
                    ...
                    return [<td key={`sisa-${entity}`} className={`w-32 min-w-32 max-w-32 border-r border-t px-3 py-3 text-right tabular-nums ${remaining < 0 ? "border-red-200 bg-red-100 font-bold not-italic text-red-700" : "border-slate-300"}`}>{number(remaining)}</td>];
                  }
