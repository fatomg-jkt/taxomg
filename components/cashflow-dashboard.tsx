...
function CashflowOverview({ data, onSave, saving }: { data: CashflowData; onSave: () => void; saving: boolean }) {
  ...
  return <><Card className="rounded-3xl"><CardContent className="flex flex-wrap gap-3 p-4">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      ...
      <Card className="overflow-hidden rounded-3xl border-emerald-100 bg-gradient-to-br from-white via-emerald-50/30 to-teal-50/70 shadow-sm"><CardHeader className="flex flex-row items-start justify-between gap-3"><div><CardTitle>Komposisi Departemen</CardTitle><CardDescription>Distribusi realisasi Cashflow berdasarkan departemen sesuai filter aktif.</CardDescription></div><span className="shrink-0 rounded-full border border-emerald-200 bg-emerald-100 px-3 py-1 text-[11px] font-black uppercase tracking-wide text-emerald-700">{departmentComposition.length} Departemen</span></CardHeader><CardContent>{departmentComposition.length ? <div className="relative h-80 w-full"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={departmentComposition} dataKey="value" nameKey="name" cx="42%" cy="50%" innerRadius={74} outerRadius={110} paddingAngle={3} cornerRadius={7} stroke="#FFFFFF" strokeWidth={3}>{departmentComposition.map((row,index)=><Cell key={row.name} fill={DEPARTMENT_COLORS[index % DEPARTMENT_COLORS.length]} />)}</Pie><Tooltip contentStyle={{ border: "1px solid #A7F3D0", borderRadius: 14, boxShadow: "0 12px 30px rgba(15,23,42,.12)" }} formatter={(value) => rupiah(Number(value))} /><Legend layout="vertical" verticalAlign="middle" align="right" iconType="circle" formatter={(value) => <span className="text-sm font-semibold text-slate-700">{value}</span>} /></PieChart></ResponsiveContainer><div className="pointer-events-none absolute left-[42%] top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full text-center"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-400">Total Realisasi</p><p className="mt-1 text-lg font-black tracking-tight text-slate-950">{rupiah(departmentTotal)}</p></div></div> : <div className="grid h-52 place-items-center rounded-2xl border border-dashed border-emerald-200 bg-white/60 text-sm font-semibold text-slate-500">Belum ada data realisasi departemen sesuai filter.</div>}</CardContent></Card>
    </div>
