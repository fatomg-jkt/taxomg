...
const status = (projection: number, actual: number) => !projection && !actual ? "Belum Ada Data" : actual <= projection ? "Under" : "Over";
const statusClass = (value: string) => value === "Under" ? "bg-emerald-100 text-emerald-700" : value === "Over" ? "bg-red-100 text-red-700" : "bg-slate-100 text-slate-600";
...
function CashflowOverview({ data, onSave, saving }: { data: CashflowData; onSave: () => void; saving: boolean }) {
  ...
  const statusAccent = budgetStatus === "Under" ? "border-emerald-500 bg-emerald-50/60 text-emerald-700" : budgetStatus === "Over" ? "border-red-500 bg-red-50/60 text-red-700" : "border-slate-400 bg-slate-50 text-slate-600";
  const kpis = [
    ...
    { label: "Status Cashflow", value: budgetStatus, Icon: ShieldCheck, accent: statusAccent, icon: budgetStatus === "Under" ? "bg-emerald-100 text-emerald-600" : budgetStatus === "Over" ? "bg-red-100 text-red-600" : "bg-slate-200 text-slate-500" },
  ];
