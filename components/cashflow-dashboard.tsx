...
function CashflowOverview({ data, onSave, saving }: { data: CashflowData; onSave: () => void; saving: boolean }) {
  ...
  const statusAccent = budgetStatus.startsWith("Under") ? "border-emerald-500 bg-emerald-50/60 text-emerald-700" : budgetStatus.startsWith("Over") ? "border-red-500 bg-red-50/60 text-red-700" : "border-slate-400 bg-slate-50 text-slate-600";
  const kpis = [
    ...
    { label: "Status Cashflow", value: budgetStatus, Icon: ShieldCheck, accent: statusAccent, icon: budgetStatus.startsWith("Under") ? "bg-emerald-100 text-emerald-600" : budgetStatus.startsWith("Over") ? "bg-red-100 text-red-600" : "bg-slate-200 text-slate-500" },
  ];
