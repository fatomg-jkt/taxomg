...
function CashflowOverview({ data, onSave, saving }: { data: CashflowData; onSave: () => void; saving: boolean }) {
  ...
  return <><Card className="rounded-3xl"><CardContent className="flex flex-wrap gap-3 p-4">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      ...
      <Card className="rounded-3xl"><CardHeader><CardTitle>Komposisi Departemen</CardTitle><CardDescription>Distribusi realisasi Cashflow berdasarkan departemen sesuai filter aktif.</CardDescription></CardHeader><CardContent>{departmentComposition.length ? <div className="relative h-80 w-full"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={departmentComposition} dataKey="value" nameKey="name" cx="42%" cy="50%" innerRadius={72} outerRadius={106} paddingAngle={2}>{departmentComposition.map((row,index)=><Cell key={row.name} fill={DEPARTMENT_COLORS[index % DEPARTMENT_COLORS.length]} />)}</Pie><Tooltip formatter={(value) => rupiah(Number(value))} /><Legend layout="vertical" verticalAlign="middle" align="right" formatter={(value) => <span className="text-sm text-slate-700">{value}</span>} /></PieChart></ResponsiveContainer><div className="pointer-events-none absolute left-[42%] top-1/2 -translate-x-1/2 -translate-y-1/2 text-center"><p className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Total Realisasi</p><p className="mt-1 text-lg font-black text-slate-900">{rupiah(departmentTotal)}</p></div></div> : <div className="grid h-52 place-items-center rounded-2xl border border-dashed border-slate-200 text-sm font-semibold text-slate-500">Belum ada data realisasi departemen sesuai filter.</div>}</CardContent></Card>
    </div>
